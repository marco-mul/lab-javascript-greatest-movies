...
function getAllDirectors(movies) {
  const allDirectors = movies.map((directorName) => directorName.director)
    return allDirectors;
}
...
function howManyMovies(movies) {
  const moviesBySpeilberg = movies.filter(function (item) {
    return item.director === 'Steven Spielberg' && item.genre.contains == 'Drama'
  });
  return moviesBySpeilberg.length
}

// Iteration 3: All scores average - Get the average of all scores with 2 decimals
function scoresAverage(movies) {
  const allScores = movies.map((movieScores) => movieScores.score)
  

  const totalScore = allScores.reduce(function (accumulator, currentValue) {
...
function dramaMoviesScore(movies) {
  ...
  const allDramaScores = dramaMovies.map((dramaScores) => dramaScores.score)
  

  const totalDramaScores = allDramaScores.reduce(function (accumulator, currentValue) {
  return accumulator + currentValue;
  },0);
  

  let averageDramaScore = totalDramaScores/dramaMovies.length
  averageDramaScore = +averageDramaScore.toFixed(2)
...
function orderByYear(movies) {
  const arrayByYear = movies.year.sort(function (a, b) {
    return a - b
  })
}
...
function orderAlphabetically(movies) {
  const alphabeticalArray = movies.title.sort(function (a, b) {
    return b - a
  })
  for (let i= 0; i<20; i++) {
    return movies[i]
